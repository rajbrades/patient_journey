...
export default function GoalMappingsPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href="/admin/goals" className="text-sm text-blue-600 hover:underline">
          &larr; Back to Goals
        </Link>
      </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Manage recommended tests for this goal</p>
          </div>
        </div>
        <button
          onClick={() => {
        ...
        <div className="mb-6 rounded-xl border border-blue-200 bg-blue-50 p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, relevance: e.target.value as 'primary' | 'secondary' })
              }
              className="rounded-lg border px-3 py-2 text-sm bg-white"
            >
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button
              ...
              className="rounded-lg bg-gray-200 px-4 py-1.5 text-sm text-gray-700 hover:bg-gray-300"
            >
              Cancel
            </button>
          </div>
      ...
      {mappings.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-gray-300 p-8 text-center text-gray-500">
          No tests mapped to this goal yet. Click &quot;Add Test&quot; to get started.
        </div>
      ) : (
        <div className="space-y-3">
          {mappings.map((mapping, idx) => (
            <div key={mapping.id} className="rounded-xl border border-gray-200 bg-white p-4">
              {editing === mapping.id ? (
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-gray-900">{mapping.test.name}</h3>
                        <span
                      ...
                      {mapping.rationale && (
                        <p className="text-sm text-gray-500 mt-0.5">{mapping.rationale}</p>
                      )}
                    </div>
                  </div>
