...
export default function AdminGoalsPage() {
  ...
  async function handleCreate() {
    ...
    if (res.ok) {
      ...
      const res2 = await fetch("/api/admin/goals");
      setGoals(await res2.json());
      setLoading(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    if (!confirm("Delete this goal? This will also remove all test mappings."))
      return;
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {goals.map((goal) => (
          <div key={goal.id} className=" border border-gray-200 bg-white p-4">
            {editing === goal.id ? (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {goal.category} &middot; {goal.slug}
                    </p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Link
                    href={`/admin/goals/${goal.id}/mappings`}
