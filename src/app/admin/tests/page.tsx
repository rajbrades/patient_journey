...
export default function AdminTestsPage() {
  ...
  async function handleCreate() {
    ...
    if (res.ok) {
      ...
      const res2 = await fetch("/api/admin/tests");
      setTests(await res2.json());
      setLoading(false);
    }
  }
  ...
  const formFields = (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <input
        placeholder="Test Group (e.g. ahb)"
        value={form.test_group}
        onChange={(e) => setForm({ ...form, test_group: e.target.value })}
        className=" border px-3 py-2 text-sm"
      />
      <input
        placeholder="Biomarkers (comma separated)"
        value={form.biomarkers}
        onChange={(e) => setForm({ ...form, biomarkers: e.target.value })}
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {tests.map((test) => (
          <div key={test.id} className=" border border-gray-200 bg-white p-4">
            {editing === test.id ? (
              <>
                {formFields}
                <div className="mt-3 flex gap-2">
